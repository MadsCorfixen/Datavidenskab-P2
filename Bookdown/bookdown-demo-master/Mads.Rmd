...
res <- replicate(n = 100, {
  ...
  boot_t <- replicate(n = bootstraps, {
    boot_x <- rbeta(n, shape1 = stik1_shape1, shape2 = stik2_shape2)
...
type_1_fejl <- replicate(n = reps, {
  ...
  for(i in 1:bootstraps){
    ...
    t_boot[i] <- ((mean(boot_x) - mean(boot_y)) -
                      (mean(stik1) - mean(stik2))) /
                        (((sd(boot_x))^2 / n) +
                          ((sd(boot_y))^2 / m))
  }

  antal_ekstreme_vaerdier <- table(abs(t_boot) >= t_obs)[1]
  
  p_vaerdi <- (sum(antal_ekstreme_vaerdier)+1)/(length(t_boot)+1)

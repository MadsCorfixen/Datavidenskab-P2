...
Såfremt en estimator er normalfordelt eller tilnærmelsesvis normalfordelt, kan det forventes, at et estimat vil være mindre end én standardfejl fra det forventede i $68\%$ af tilfældene og mindre end to standardfejl fra i $95\%$ af tilfældene. 
...
```{r}
set.seed(11)
data <- rnorm(100000, mean = 0, sd = 1) # Normalfordelt population
n <- 1000

sample_pop <- sample(data, n) # Stikprøve fra populationen
SD_sample <- sd(sample_pop) #Stikprøve standardafvigelse 
SE_sample <- round(SD_sample/sqrt(length(sample_pop)), 4) #Stikprøve standardfejl

B <- 10000 # Antal bootstrap-repetitioner

meanFunc <- function(sample_pop,i){mean(sample_pop[i])} #Estimator

Boot_sample <- boot(data = sample_pop, statistic = meanFunc, R = B) #Bootstrap-stikprøver
SE_boot <- round(sd(Boot_sample$t), 4) #Standardfejlen for bootstrap-stikprøver

diff_procent <- round(((abs(SE_sample - SE_boot))/SE_sample) * 100, 4) #Procentvis afvigelse

```

Fra eksemplet fås, at standardfejlen for bootstrap-stikprøven er $se(\hat{\theta})=$```r SE_boot```, hvilket approksimerer stikprøvens standardfejl på $\text{se}(\hat{\mu})=$```r SE_sample```, med en forskel i dette tilfælde på ```r diff_procent```$\%$.
...
Fremgangsmåden i ovenstående afsnit gentages $B$ gange, hvilket giver $B$ nye teststørrelser.   

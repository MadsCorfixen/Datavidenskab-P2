...
I kodetstykket nedenfor laves en tilfældig population med en standard normalfordeling. Ud fra populationen udtages en tilfældig stikprøve. Der udregnes standard fejlen for stikprøven. Derefter laves $10,000$ bootstrap-stikprøver, hvor standard fejlen for bootstrap-stikprøven udrenges 
...
library(boot)
set.seed(11)
data <- rnorm(100000, mean = 0, sd = 1) # Tilfældig population

n <- 1000

sample_pop <- sample(data, n) # Stikprøve fra populationen
SD_sample <- sd(sample_pop)
SE_sample <- SD_sample/sqrt(length(sample_pop))

B <- 10000 # Bootstrap-repetitioner

sample_boot <- replicate(B, {
  x <- sample(sample_pop, size = n, replace = TRUE)
}) # Bootstrap over populations stikprøven
SD_boot <- sd(sample_boot)
SE_boot <- SD_boot/sqrt(n)
c(SE_sample, SE_boot) # Sammenligning af SE
meanFunc <- function(sample_pop,i){mean(sample_pop[i])}

boot_samp <- boot(data = sample_pop, statistic = meanFunc, R = 10000)
boot_samp
```

Det ses at standard fejlen for bootstrap-stikprøven approximere stikprøven for populationen.  

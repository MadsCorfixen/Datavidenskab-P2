...
__FIXME__ Gerne forklaring mellem hver linje. $\hat{\theta}$ - Værs'go Ronni. 

I udledningen anvendes at differencen $\theta - \hat\theta$ er tilnærmelsevist den samme som $\hat\theta - \hat\theta^*$. Derfor gælder følgende:
...
&= P[q_{(\alpha/2)} - \hat\theta \leq \hat{\theta}^* - \hat\theta \leq q_{(1-\alpha/2)} - \hat\theta] \\
&= P[\hat\theta - q_{(\alpha/2)} \geq \hat\theta - \hat{\theta}^* \geq \hat\theta - q_{(1-\alpha/2)}] \\
...
\end{aligned}
$$
Dette giver den nedre grænse $2\hat\theta - q_{(\alpha/2)}$ og den øvre grænse $\theta \geq 2\hat\theta - q_{(1-\alpha/2)}$.

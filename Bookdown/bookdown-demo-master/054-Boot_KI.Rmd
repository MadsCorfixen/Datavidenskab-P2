...
Denne type konfidensinterval ud fra bootstrap er også kendt som _reversed percentile interval_. 
Pivot(Jeg har ingen ide om hvad det betyder?!) konfidensintervallet giver at forholdet mellem $\theta - \hat\theta$ er tilnærmelsesvis det samme som forholdet mellem $\hat\theta - \hat\theta^*$, derfor gælder følgende:



$$
\begin{aligned}
0.95 &\approx P[\hat\theta_{(\alpha/2)} \leq \hat\theta^* \leq \hat\theta^*_{(1-\alpha/2)}] \\
&= P[\hat\theta^*_{(\alpha/2)} - \hat\theta \leq \hat\theta^* - \hat\theta \leq \hat\theta^*_{1-\alpha/2} - \hat\theta] \\
&= P[\hat\theta - \hat\theta^*_{(\alpha/2)} \geq \hat\theta - \hat\theta^* \geq \hat\theta - \hat\theta^*_{1-\alpha/2}] \\
&\approx P[\hat\theta - \hat\theta^*_{(\alpha/2)} \geq \theta - \hat\theta \geq \hat\theta - \hat\theta^*_{1-\alpha/2}] \\
&= P[2\hat\theta - \hat\theta^*_{(\alpha/2)} \geq \theta \geq 2\hat\theta - \hat\theta^*_{1-\alpha/2}] \\
\blacksquare
\end{aligned}
$$
$$
\begin{aligned}
0.95 &\approx P[q_{(\alpha/2)} \leq q \leq q_{(1-\alpha/2)}] \\
&= P[q_{(\alpha/2)} - \hat\theta \leq q - \hat\theta \leq q_{1-\alpha/2} - \hat\theta] \\
&= P[\hat\theta - q_{(\alpha/2)} \geq \hat\theta - q \geq \hat\theta - q_{1-\alpha/2}] \\
&\approx P[\hat\theta - q_{(\alpha/2)} \geq \theta - \hat\theta \geq \hat\theta - q_{1-\alpha/2}] \\
&= P[2\hat\theta - q_{(\alpha/2)} \geq \theta \geq 2\hat\theta - q_{1-\alpha/2}] \\
\blacksquare
\end{aligned}
$$
Dette giver den nedre kvartil $= 2\hat\theta - q_{(\alpha/2)}$ og øvre kvartil $=\theta \geq 2\hat\theta - q_{1-\alpha/2}$
Basic metoden benytter nedenstående formel til udregning af konfidensintervaller.
...
Beviset for formlen til konfidensintervalelt for T-metoden, følger samme metode som i afsnittet for basic-metoden. 

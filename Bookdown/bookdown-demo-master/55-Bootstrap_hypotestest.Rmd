...
Er antagelserne ikke opfyldt kan bootstrap benyttes til at udføre t-test. Benyttes bootstrap til at udføre en t-test, kaldes det i det følgende for en bootstrap-test. 

#### Parret bootstrap-test

Fremgangsmåden for en parret tosidet bootstrap-test vil blive gennemgået i dette afsnit.
...
Så opsættes der en nulhypotese, $H_0:~\mu = 0$, hvor $\mu$ angiver den sande middelværdi for differencerne for $X$ og $Y$, og en alternativ hypotese, $H_1:~\mu\neq 0$, samt et signifikansniveau, $\alpha = 0.05$. 
...
\begin{align}
p_{mindre} &= \frac{\text{antal gange }\{t^*<t_{obs}\}}{B} \\
p_{større} &= \frac{\text{antal gange }\{t^*>t_{obs}\}}{B} \\
...
I dette afsnit vil fremgangsmåden for en uparret tosidet bootstrap-test blive gennemgået. Fremgangsmåden startes med at der er to uafhængige stikprøver, $X=[x_{1},~x_{2},~...,~x_{n}]$ og $Y=[y_{1},~y_{2},~...,~y_{m}]$.

På baggrund af forskellen i de to stikprøvers middelværdi, er det muligt at udregne en teststørrelse, $t_{obs}=\hat{X}-\hat{Y}$.

Så opsættes der en nulhypotese, $H_0: \mu_{_X} = \mu_{_Y}$, hvor $\mu_{_X}$ og $\mu_{_Y}$ er de sande middelværdier for populationerne, hvorfra stikprøverne blev udtrukket og en alternativ hypotese, $H_1: \mu_{_X} \neq \mu_{_Y}$, samt et signifikansniveau, $\alpha = 0.05$. 

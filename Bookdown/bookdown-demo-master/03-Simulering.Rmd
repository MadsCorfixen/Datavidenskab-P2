...
```{r}
lcg1 <- lcg(modulus = 64, faktor = 33, tilvaekst = 12, seed = 57, iterationer = 20)
lcg1
```
...
```{r}
lcg2 <- lcg(modulus = 86436, faktor = 1093, tilvaekst = 0, seed = 12, iterationer = 1000)
...
```{r}
lcg3 <- lcg(modulus = 86436, faktor = 1093, tilvaekst = 18257, seed = 12, iteration = 100)
```
...
```{r}
Z1 <- sqrt(-2*log(U1))*cos(2*pi*U2) # Opretter Z1 vha. formlen for Box-Muller
Z2 <- sqrt(-2*log(U1))*sin(2*pi*U2) # Opretter Z2 vha. formlen for Box-Muller
```
...
```{r}
OutVals1 <- boxplot(Z1, plot = FALSE)$out # Output'er outliers i Z1
outliers_Z1 <- length(OutVals1) # Tæller antal outliers i Z1

OutVals2 <- boxplot(Z2, plot = FALSE)$out # Output'er outliers i Z2
outliers_Z2 <- length(OutVals2) # Tæller antal outliers i Z2

outliers_andel_Z1 <- outliers_Z1/length(Z1) # Andelen af outliers i Z1
outliers_andel_Z2 <- outliers_Z2/length(Z2) # Andelen af outliers i Z2
```
...
```{r korrelationstest, echo = FALSE, fig.align='center', fig.cap='Graf for uafhængigheden mellem Z1 og Z2.'}
plot(Z2 ~ Z1, main = "",

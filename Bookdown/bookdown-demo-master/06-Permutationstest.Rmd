...
I dette afsnit vil det eftervises, at for en t-test foretaget på en ikke-normaltfordelt stikprøve, kan resultaterne ikke altid antages at være retvisende. Først udtrækkes to stikprøver på de skæve populationer i afsnit \@ref(pop-sim).
...
```{r , fig.align="center", echo=FALSE, fig.cap= "Stikprøve fra højreskæv betafordeling hvor n = 100, &alpha; = 2, &beta; = 8"}
hist(Stik1, main = "", ylab = "", xlab = "")
```

```{r , fig.align="center", echo=FALSE, fig.cap= "Stikprøve fra venstreskæv betafordeling hvor n = 100, &alpha; = 8, &beta; = 2."}
...
Udfra t-testen fåes et konfidensinterval på [```r t1$conf.int```]. Med et signifikansniveau på $5\%$, vil den observerede forskel mellem populationernes middelværdier, ifølge t-testen, ligge i intervallet i $95\%$ af tilfældene. Dækningsgraden af dette konfidensinterval kan undersøges ved at trække nye stikprøver fra populationerne. Dette gøres i nedenstående kodestykke ved at udregne den nye observerede forskel i middelværdi og sammenligne med konfidensintervallet. Hvis den observede forskel er indeholdt i konfidensintervallet defineres det som en succes, ellers defineres det som en fiasko.
...
```{r , fig.align="center", fig.cap= "Andel af succeser og fiaskoer", echo=FALSE}

...
### Ikke-parret T-test for ikke-normalfordelt stikprøve

FIXME: Skriv afsnittet om på baggrund af Mikkels kommentarer fra vejledermøde 14/4-2020.

Foretages en t-test på en ikke-normaltfordelt stikprøve, kan resultaterne ikke altid antages at være retvisende. Dette vil nu vises. Først opstilles to ikke-normalfordelte populationer hvor, at nulhypotesen er $H_0: \mu_1 - \mu_2 = 0$ og en alternativ hypotese er $H_1 : \mu_1 - \mu_2 \neq 0$. Der vil undersøges ved hjælp af t-test om middelværdien af de to populationer er ens.
...
```{r, echo=FALSE}
hist(Pop1, main = "Population 1", ylab = "", xlab = "")
hist(Pop2, main = "Population 2", ylab = "", xlab = "")
```


Der udtrækkes en stikprøve for hver population og udfra disse udføres der to-sidet uparret t-test, ved hjælp af den indbygget funktion `t.test`.
```{r}
#To stikprøver trukket fra populationerne
Stik1 <- sample(Pop1, 100)
Stik2 <- sample(Pop2, 100)
```
...
Udfra t-testen fåes et konfidensinterval på [```r t1$conf.int```]. Med et signifikansniveau på $5\%$ vil det sige at ifølge t-testen vil den observerede forskel mellem populationernes middelværdier ligge i dette interval i $95\%$ af tilfældene. Dækningsgraden af dette konfidensinterval kan undersøges ved at trække nye stikprøver fra populationerne, i alt $10,000$ gange. Dette gøres ved at udregne den nye observerede forskel i middelværdi og sammenligne med konfidensintervallet. Hvis den observede forskel er indeholdt i konfidensintervallet er det en succes, ellers er det en fiasko.

```{r}
#Undersøg dækningsgrad af konfidensintervallet ved udtræk af nye stikprøver.
res <- replicate(1000, {
  x1 <- sample(Pop1, 100)
  x2 <- sample(Pop2, 100)
...
tf <- table(res)
pie(x = tf, labels = c("Fiasko","Succes"), main = "Andel af succeser")
```

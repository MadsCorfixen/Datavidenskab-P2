...
KI_Percentil <- c(Nedre = SortedBoots[NedrePercentil], Oevre = SortedBoots[OevrePercentil])
...
for(n in seq(4, 100, 4)){
  res <- replicate(n = 100,{
    ...
    boot_stik <- boot::boot(data = stik, statistic = meanFunc, R = 100)
    interval <- boot::boot.ci(boot.out = boot_stik, conf = KI_niveau, 
    ...
    # Undersøger om den sande middelværdi ligger i konfidensintervallet

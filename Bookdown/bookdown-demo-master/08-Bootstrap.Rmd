...
Bootstrap opererer med tilbagelægning, så der er en sandsynlighed for at et givent datapunkt bliver udtaget mere end en gang, samtidig er der en sandsynlighed for at et datapunkt slet ikke bliver udvalgt. Hver bootstrap-stikprøve har størrelsen $n$, altså den samme størrelse som stikprøven. (FIXME her kunne andelen af oprindelige observationer passe godt ind) I alt bliver der genereret $b$ antal bootstrap-stikprøver, som der hver især udføres statistisk inferens på. Ud fra disse estimater, kan der estimeres en stikprøvefordeling, hvor der udfra kan udføres yderligere statistisk inferens [@TDSBootstrap]. (FIXME Her kunne antal resamples passe godt ind)

Fordelen ved bootstrap er, at selvom man kun har en stikprøve fra den underliggende population, er der stadig mulighed for at estimere stikprøvefordelingen, uden at der kræves yderligere stikprøver fra populatonen. 

```{r, figur-Bootstrap_illustration, out.width='75%', fig.align='center', fig.cap = "Her er illustreret forskellen mellem at finde den teoretiske stikprøvefordeling ved hjælp af mange stikprøver fra population, og måden hvorpå stikprøvefordelingen kan findes ved hjælp af kun en stikprøve, der udføres bootstrap på", echo = FALSE}
knitr::include_graphics('images/normalvboot.PNG')
```

Der er to hovedårsager til at benytte bootstrap, som beskrevet i [@BootYouTube1]. For det første, hvis stikprøven ikke er stor, og stikprøvefordelingen derfor ikke kan antages at være normalfordelt. For det andet, hvis metoden til at beregne teststørrelsens standardfejl er teoretisk avanceret. Eksempelvis er standardfejlen for middelværdien nem at løse, $\hat \sigma_{MV} = \frac{\text{sd}}{\sqrt n}$, mens det ikke er tilfældet, hvis det i stedet er er afstanden mellem to percentiler, der estimeres.

__Beregning af standardfejl__ 

Det følgende afsnit at primært skrevet på baggrund af [@TDSBootstrap]

Standardafvigelsen for en estimator, beskrives som _standard fejl_. Den er et udtryk for, hvor stor en afvigelse der er fra den aktuelle parameter til stikprøvens estimat. Jo mindre _standard fejlen_ er, desto mindre er afvigelsen mellem estimatet og parameteren, derved en højere nøjagtighed for estimatet. __FIXME__ Som udgangspunkt vil en stikprøves middelværdi, $\bar{X}$, aldrig være $= \mu$, fordi der ved udtagning af en stikprøve, i hvert tilfælde vil være variabilitet

$\sigma_\bar{x} = \frac{\sigma}{\sqrt{n}}$, hvor $\sigma$ er standardafvigelsen for populationen og n er størrelsen på stikprøven. 

__FIXME__ Når der arbejdes med data udover det teoretiske, vil standardafvigelsen for populationen, $\sigma$, altid være ukendt. Derfor anvendes den _estimeret standardfejl_ ofte som måling i stedet for populationens standardafvigelse. Den _estimeret stadardfejl_ bruger stikprøvens standardafvigesle, _S_ som et estimat for standardafvigelsen for populationen: 

$SE_\bar{X} = \sigma_\bar{x} \approx \frac{S}{\sqrt{n}}$, hvor $S = \sum_{i=1}^{n} \frac{(x_i - \bar{x})^2}{n - 1}$, er stikprøvens standardafvigelse for n uafhængige datapunkter, og n er størrelsen på stikprøven.

Såfremt en estimator er normalfordelt eller tilnærmelsesvis er normalfordelt, kan det forventes at et estimat vil være mindre end en standardafvigelse fra det forventet i $68%$ af tilfældene og mindre end to standardfejl fra i $95%$ af tilfældene. 
...
Dog er dette ikke altid lige til i virkeligheden, oftest er der ikke tilstrækkelig informationer om populationen eller fordeligen af denne, samtidig kræver det at der er nogle specifikke krav som er tilstede. Anvendelse af standardfejl i forbindelse med bootstrap-stikprøver...

__Antal resamples__

Med den eksisterende computerkraft tilgængelig i dag, anbefales der mindst 10,000 resamples, derved $b = \geq 10,000$, for at få et nøjagtigt estimat. Det kan undres, at der ikke vælges at genere et endnu større antal bootstrap-stikprøver end de 10,000. Grunden til dette er, at bootstrap-stikprøven generes ud fra den obseveret data med $n$ størrelse, og der vil derfor ikke skaffes yderlige information om populationen med et større $b$. Det eneste der vil opnås ved et højere $b$, er et lidt bedre estimat. (FIXME: Kilde? Er fra 10:20 https://www.youtube.com/watch?v=O_Fj4q8lgmc&list=PLqzoL9-eJTNDp_bWyWBdw2ioA43B3dBrl&index=2&t=0s)

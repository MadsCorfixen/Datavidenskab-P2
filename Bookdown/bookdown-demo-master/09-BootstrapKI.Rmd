...
__FIXME__ Bedre begrundelse for, hvorfor vi har flere forskellige metoder på konfidensintervaller med. Er der nogle, der er bedre end andre? 
...
$$\text{KI}_p = \left[\theta^{*}_{(\alpha/2)},~\theta^{*}_{(1-\alpha/2)}\right]$$
...
$$\text{KI}_b = \left[2\hat\theta- \theta^{*}_{(1-\alpha/2)}, ~ 2\hat\theta- \theta^{*}_{(\alpha/2)}\right]$$ 

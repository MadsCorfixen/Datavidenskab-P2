...
En computer fungerer i hovedtræk ved at den modtager et input, der så bearbejdes af en algoritme, som derefter returnerer et output. Der findes ingen algoritmer, som er i stand til at genererer FIXME(på engelsk kaldes det truly random numbers, dækker ægte? eller rigtig? eller bare tilfældig?)ægte tilfældige tal, fordi gives det samme input, til den samme algoritme, vil resultatet være det samme output som tidligere. Årsagen til dette er, at en computer fungerer på baggrund af matematik og er derfor deterministisk. Det er dog muligt ved hjælp af en beregningsmodel, at skabe en abstraktion af ægte tilfældighed. Denne model kaldes _Pseudo Random Number Generator_ (PRNG).
...
Lineær Kongruens Generator, på engelsk Linear Congruential Generator eller LCG, er en af de mange PRNG generatorer. Denne algoritme er en af de ældste og nemmeste at implementere. LCG danner en sekvens af tal ved rekursion, og kræver kun få parametre. Helt specifikt er algoritmen angivet ved

>```{definition}
> $X_{n+1} = (a \cdot X_n + c) \mod m$. 
> Hvor $X_n$ er en liste af tal fra $X_0$ til $X_n$, $m$ er modulus, $a$ er en faktor, $c$ er tilvæksten, og $X_0$ er startpunktet.
>```

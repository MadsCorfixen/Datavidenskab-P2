...
```{r, out.width='75%', fig.align='center', fig.cap="PRNG fordelinger", echo = FALSE}
 knitr::include_graphics("images/prngfordeling.PNG")
```
...
$$
Hvor $Cov$ angiver kovariansen og $std$ angiver standardafvigelsen.

Korrelation fortæller hvor stor en lineær sammenhæng der er mellem to variabler. Denne værdi kan ligge i et interval fra [-1, 1], hvor $-1$ påviser en perfekt negativ lineær sammenhæng, $1$ påviser en perfekt positiv lineær sammenhæng, mens $0$ viser enten ingen samemnhæng eller blot ingen lineær sammenhæng. 

```{r}
cor(Z1, Z2)
```

Et resultat på afrundet $-0.00048$, hvilket næsten er 0, viser altså også at der ingen korrelation er mellem ```Z1``` og ```Z2```, hvilket giver en god indiktion for, at de er uafhængige.

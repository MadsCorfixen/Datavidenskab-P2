...
lcg <- function(m, a, c, seed, n) {
  ...
  r <- numeric(n) #Sørger for at n er numerisk
...
```{r}
plot(lcg1, main = "Eksempel på en ikke tilfældig LCG sekvens", xlab = "Indeks", ylab = "Sekvens værdier" )
```
...
hist((lcg2 + 0.05)/ 86436, main = "LCG Uniformfordeling", xlab = "Sekvens værdier", ylab = "Frekvens")
```
...
```{r}
plot(lcg2, main = "2d plot af LCG sekvens", xlab = "Indeks", ylab = "Sekvens værdier")
```
...
```{r}
lcg3 <- lcg(86436, 1093, 18257, 12, 1000)

plot(lcg3, main = "'Tilfældig' LCG Sekvens", xlab = "Indeks", ylab = "Sekvens værdier")
```

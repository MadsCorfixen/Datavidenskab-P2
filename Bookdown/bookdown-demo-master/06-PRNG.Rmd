...
FIXME Kilde? https://qualityandinnovation.com/2015/03/03/a-linear-congruential-generator-lcg-in-r/
...
FIXME Billeder af poor parameters, https://rpubs.com/aaronsc32/linear-congruential-generator-r 
...
<!-- Nedenfor ses et eksempel på et stykke kode der vil danne en sekvens af LCG tal. Hvis man for eksempel kørte koden med parametrene: lcg(9, 2, 0, 3) vil man få en sekvens der uendeligt returnerer værdierne 3, 6, 3, 6, 3, 6... Dette er også jf. i FIXME figuren ovenover. -->
...
<!--         print(seed) -->
<!-- ``` -->
FIXME De følgende simuleringer er baseret på kilden http://www.sci.csueastbay.edu/~btrumbo/Stat3401/Hand3401/CongGenIntroB.pdf, som også går mere i dybden med hvordan man kan undersøge om en LCG, generer tilfældige værdier, og hvilke parametre der skal vælges for at opfylde dette.

Nedenfor ses et eksempel på en LCG generator. Her vælges værdierne for parametrerne, og rekursionen køres igennem m gange, som i dette tilfælde vil give os 100 værdier. Her ses et eksempel på en LCG sekvens med en periode på 16. Perioden fortæller, hvor mange værdier generatoren returnerer inden den gentager sig selv.  
...
Hvis man plotter denne sekvens i et 2d punktplot, vil man også hurtigt se at værdierne i virkeligheden nok ikke er helt tilfældige. Selv hvis man ændrer antallet af gange LCG køres igennem, (m-værdien), vil resultatet ikke ændre sig. 
...
Nedenunder ses et eksempel på hvordan man kan bruge LCG algortimen til at lave en uniformfordeling i intervallet [0, 1]. Her kan det også nævnes at hvis man plotter de værdierne algoritmen returnerer i et 2d punktplot, vil man se en tydeligere "tilfældig" fordeling, dog vil de fleste stadig kunne se et mønster.
...
Nedenunder ses et eksempel på LCG, hvor der returneres en sekvens, der plottet i et 2d punktplot, ser tilfældig ud. 
...
I dette afsnit beskrives Box-Muller transformation, hvilket er en metode til at generere normalfordelte tal ud fra uniformt fordelte tal. Dette gøres for at belyse, hvordan en computer kan generere tilsyneladende tilfældige tal.

### Generelt om algoritmen

FIXME: Skriv generelt om BMT.

### Eksempel i R

I dette følgende efterprøves Box-Muller transformationens regler ved hjælp af R.
...
Det ses på boksplottet, at der er en indikation på normalfordeling med henvisning til de fire krav et boksplot af en normalfordeling opfylder. 
De fire krav er:
...
3) Der er lige mange outliers over øvre kvartil og under nedre kvartil.
...
Dette giver en andel af outliers i ```Z1``` på ```r outliers_andel_Z1``` og i ```Z2``` på ```outliers_andel_Z2```. Dette svarer til cirka $0,7 \%$ outliers i både ```Z1``` og ```Z2```, hvilket stemmer overens med punkt 4.
...
Det ses tydeligt på histogrammerne, at de tilnærmelsesvist er normalfordelte, samt at de har en middelværdi på 0 og en standardafvigelse på 1, og altså, at ```Z1``` og ```Z2``` er standard normalfordelte.

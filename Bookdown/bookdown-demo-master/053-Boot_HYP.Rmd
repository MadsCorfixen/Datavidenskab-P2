...
```{r}
t_obs <- (mean(stik1) - mean(stik2))/((sd(stik1)/sqrt(n1))+(sd(stik2)/sqrt(n2)))
t_obs
```
...
boot_t <- replicate(B, {
    ...
    boot_test <- (mean(bootx) - mean(booty))/((sd(bootx)/sqrt(n1))+(sd(booty)/sqrt(n2)))
})
```

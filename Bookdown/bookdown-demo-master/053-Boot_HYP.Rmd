...
Lad to uafhængige uparrede stikprøver, $X=[x_{1},~x_{2},~...,~x_{n_1}]$ og $Y=[y_{1},~y_{2},~...,~y_{n_2}]$, med ens varians, være givet. 

```{r}
stik1 <- rbeta(n1, alfa1, beta1)
stik2 <- rbeta(n2, alfa2, beta2)
```
```{r fig.cap = "To uafhængige uparrede stikprøver", echo = FALSE}
par(mfrow = c(1,2))
...
```{r boothypuparret, fig.cap = "Bootstrap teststørrelse fordelingen, hvor den blå linje markerer den observeret teststørrelse."}
hist(boot_t, main = "", ylab = "Antal observationer", xlab = "Bootstrap teststørrelse", xlim = c(-0.3, 1))
abline(v = t_obs, col = "blue")
```

Herefter kan p-værdien udregnes, på tilsvarende måde som i det forrige eksempel.
...
Med en p-værdi på ```r p_vaerdi```, kan $H_0$ forkastes, da der ikke er evidens for at middelværdierne for de to populationer er ens. 
